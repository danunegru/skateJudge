...
@Component({
  ...
  styleUrl: './a3.component.scss'  // Changed from styleUrls to styleUrl
})
export class A3Component implements OnInit, AfterViewInit {
  // Basic properties
  ...
  // A3-specific elements
  elements: string[] = [
    'Dreiersprung mit Mohawkanlauf',
    'Salchow',
    'Rittberger',
    'Zweibeinpinrouette (min. 5 Umdrehungen)',
  ...
  confirmFinalScores(): void {
    ...
    if (missingScores.length > 0) {
      ...
      this.totalScoresNeeded = this.prueflingeForExam.length * this.elements.length;
  ...
  finishScoring(): void {
    ...
    console.log('A3 Bewertung abgeschlossen!');
  }
  ...
  private saveCurrentScore(): void {
    if (this.currentPopupData && this.currentScore !== null && this.currentScore !== undefined) {
      ...
      console.log(`A3 Score saved: Element ${elementIndex}, Athlete ${athleteIndex + 1}, Score: ${this.currentScore}`);
    }
  }
}
